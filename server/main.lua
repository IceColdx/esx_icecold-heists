...
RegisterServerEvent("esx_icecold-heists:startRobbery")
...
RegisterServerEvent("esx_icecold-heists:keycardCheck")
AddEventHandler("esx_icecold-heists:keycardCheck", function(bankId)
	local source = source
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if xPlayer.getInventoryItem('keycard_gold').count >= 1 then
		...
		TriggerClientEvent("esx_icecold-heists:cardTime", source, bankId, _cardTime, _dyeChance)
	...
	elseif xPlayer.getInventoryItem('keycard_green').count >= 1 then
		...
		local _cardTime = nil
		TriggerClientEvent("esx_icecold-heists:cardTime", source, bankId, _cardTime)
		TriggerClientEvent("esx:showNotification", source, _U('no_keycard'))
	end
end)

RegisterServerEvent("esx_icecold-heists:lockpickDoor")
AddEventHandler("esx_icecold-heists:lockpickDoor", function(bankId)
	local source = source
	local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer.getInventoryItem('lockpick').count >= 1 then
		xPlayer.removeInventoryItem('lockpick', 1)
		TriggerClientEvent("esx_icecold-heists:desks", source, bankId)
	else
...
AddEventHandler("esx_icecold-heists:deskOpen", function(bankId)
  TriggerClientEvent("esx_icecold-heists:OpenDeskDoor", -1, bankId)
end)

RegisterServerEvent("esx_icecold-heists:giveCash")
AddEventHandler("esx_icecold-heists:giveCash", function(bankId, totalCash)
	local source = source
	local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterServerEvent('esx_icecold-heists:giveItems')
AddEventHandler('esx_icecold-heists:giveItems', function(itemName)
    local source = source
		local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterServerEvent('esx_icecold-heists:bagTracker')
AddEventHandler('esx_icecold-heists:bagTracker', function()
	local source = source
	local xPlayer = ESX.GetPlayerFromId(source)
	local xPlayers = ESX.GetPlayers()
	...
	for i=1, #xPlayers, 1 do
		local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
		if xPlayer.job.name == 'police' then
			TriggerClientEvent('esx_icecold-heists:bagBlip', xPlayers[i])
		end
	end
end)

RegisterServerEvent("esx_icecold-heists:endRobbery")
AddEventHandler("esx_icecold-heists:endRobbery", function(bankId)
	local source = source
	local xPlayers = ESX.GetPlayers()
	local Bank = BankHeists[bankId]
	...
	TriggerClientEvent("esx_icecold-heists:delHeistBlip", -1)
